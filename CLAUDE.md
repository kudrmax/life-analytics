...
- Pure (no DB): correlation_math, time_series, value_converter, quality, pair_formatter.
...
**Multi-slot:** `measurement_slots` = global per-user slot definitions. `metric_slots` = junction table linking metrics to slots. `entries.slot_id` links entry to slot. Category lives on metric_slots (not metric_definitions) for multi-slot metrics. Slots cannot be deleted while linked to any metric (ON DELETE RESTRICT).
...
- Auto-sources: nonzero, note_count, slot_max, slot_min, rolling_avg, streak, day_of_week, month, is_workday, aw_active
