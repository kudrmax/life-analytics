...
**Checkpoints and Intervals:** `measurement_slots` = чекпоинты (контрольные точки дня), поле `description` — описание. `metric_definitions.is_checkpoint` — true = оценка (замеряется В чекпоинтах), false = факт. `metric_definitions.interval_binding` — привязка факта к времени: `daily` (весь день, без слотов), `fixed` (один интервал между чекпоинтами, 1 слот, `interval_start_slot_id`), `floating` (любой интервал, N-1 слотов). Продуктово daily и fixed эквивалентны (1 замер/день). Daily page: `split_by_checkpoints()` разбивает метрики по чекпоинтам (sandwich layout). `build_interval_label_map()` в `daily_helpers.py` строит лейблы "X → Y" для интервалов.
