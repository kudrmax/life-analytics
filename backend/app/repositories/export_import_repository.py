"""Repository for export/import SQL operations."""
...
class ExportImportRepository(BaseRepository):
    """Data access for export and import operations."""
    ...
    async def upsert_metric_slot(
        ...
        await self.conn.execute(
            """INSERT INTO metric_slots (metric_id, slot_id, sort_order, category_id)
               VALUES ($1, $2, $3, $4)
    ...
    async def disable_metric_slot(self, row_id: int) -> None:
        await self.conn.execute(
            "UPDATE metric_slots SET enabled = FALSE WHERE id = $1", row_id,
        )
    ...
    async def insert_metric_slot_on_fly(
        self, metric_id: int, slot_id: int, sort_order: int,
    ) -> None:
        await self.conn.execute(
