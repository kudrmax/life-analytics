...
| `low_streak_resets` | count(zeros) < 2 в streak данных | bad | Серия не сбрасывалась, монотонный рост создаёт ложные корреляции |
...
**Правило:** streak должен сброситься (значение = 0) хотя бы `_MIN_STREAK_RESETS` = 2 раза за период. Если count(zeros) < 2 — источник помечается как `low_streak_reset_sources`, и все пары с его участием получают `low_streak_resets` (severity = `bad`).

**Проверка на уровне источника** (source-level), не пары: если streak не сбрасывался, он ненадёжен в любой паре.

## Код

`QualityIssue` enum, `_determine_quality_issue()`, `low_var_sources`, `binary_sources`, `low_streak_reset_sources`, `_check_small_binary_group()`, `_BINARY_TYPES`, `_build_contingency_table()`, `_log_hypergeometric()`, `_fisher_exact_p()` → `backend/app/routers/analytics.py`
