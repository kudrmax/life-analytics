...
| `low_streak_resets` | streak монотонно не убывает на общих датах пары | bad | Серия не сбрасывалась, монотонный рост создаёт ложные корреляции |
...
**Правило:** streak-значения на общих датах пары не должны быть монотонно неубывающими. Если `vals[i] <= vals[i+1]` для всех i — streak монотонно растёт на тех же данных, по которым считается Pearson → ложная корреляция.

**Проверка на уровне пары** (pair-level, как `low_binary_data_points`): монотонность проверяется только на **общих датах** (пересечении). Это важно, потому что streak может сбрасываться за полный период, но при пересечении с источником, данные которого начинаются позже (например, rolling_avg), сбросы выпадают и streak становится монотонным.
